...
function DashboardContent({ darkMode }) {
  ...
  const prepareChartData = (data) => {
    const chartData = data.map((process) => ({
      ...
      collection: parseInt(
        process.stats.totalMonthCollection.replace(/,/g, "")
      ),
    ...
    const efficiency = data.map((process) => {
      ...
      return {
        name: process.name.replace("_paid", "").toUpperCase(),
        efficiency:
          headcount > 0 ? (collection / (target / headcount)) * 100 : 0,
  ...
  const downloadExcelReports = () => {
    ...
    const excelData = chartData.map((item) => ({
      ...
      "Achievement (%)":
        efficiencyData.find((e) => e.name === item.name)?.achievement || 0,
      "Collection per Person (₹)":
        efficiencyData.find((e) => e.name === item.name)?.perPersonCollection ||
        0,
        ...
        <div className="flex space-x-4">
          <button
            ...
            className={`px-4 py-2 rounded-md ${
              viewMode === "charts" ? buttonBg : "bg-gray-200"
            } ${viewMode === "charts" ? buttonText : "text-gray-700"}`}
            disabled={!hasData}
          >
            View Charts
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-md ${
              viewMode === "table" ? buttonBg : "bg-gray-200"
            } ${viewMode === "table" ? buttonText : "text-gray-700"}`}
            disabled={!hasData}
          >
            View Table
          </button>
      ...
      {loading ? (
        ...
        <div className={`flex justify-center items-center h-64 ${textColor}`}>
          <div className="text-center">
            ...
            <p className="text-lg">
              Select date range and click Submit to load data
            </p>
          </div>
        </div>
      ) : (
        <>
          {viewMode === "charts" ? (
            <div className="space-y-8">
              ...
              <div
                className={`rounded-xl shadow-md p-6 ${bgColor} ${borderColor} border`}
              >
                <h3
                  className={`text-2xl font-semibold mb-6 ${titleColor} border-b ${borderColor} pb-2`}
                >
                  Targets vs Collections Across Processes (₹ in Lakhs)
                  <span
                    className={`block text-sm font-normal ${chartTextColor}`}
                  >
                ...
                <div className={`mt-4 text-sm ${chartTextColor}`}>
                  ...
                  <p>
                    Highest Target: ₹
                    {Math.max(
                      ...chartData.map((item) => item.target)
                    ).toLocaleString("en-IN")}
                  </p>
                  <p>
                    Total Collection: ₹
                    {chartData
                      .reduce((sum, item) => sum + item.collection, 0)
                      .toLocaleString("en-IN")}
                  </p>
                </div>
              </div>
              ...
              >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="h-[400px]">
                    <h4
                      className={`text-lg font-medium mb-4 text-center ${textColor}`}
                    >
                      ...
                      <PieChart>
                        ...
                        <Tooltip
                          formatter={(value, name, props) => {
                            const process = chartData.find(
                              (p) => p.name === props.payload.name
                            );
                            return [
                              `${value.toFixed(2)}%`,
                              `Target: ₹${process.target.toLocaleString(
                                "en-IN"
                              )}`,
                              `Collection: ₹${process.collection.toLocaleString(
                                "en-IN"
                              )}`,
                            ];
                          }}
                        ...
                        <Tooltip
                          formatter={(value, name, props) => {
                            ...
                            return [
                              ...
                              `Total: ₹${process.collection.toLocaleString(
                                "en-IN"
                              )}`,
                            ];
                          }}
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className={tableHeaderBg}>
                    <tr>
                      <th
                        className={`px-4 py-3 text-left text-xs font-medium ${tableHeaderText} uppercase tracking-wider`}
                      >
                        Process
                      </th>
                      <th
                  ...
                  <tbody className={`${tableRowBg} divide-y ${tableRowBorder}`}>
                    {chartData.map((item, index) => {
                      const efficiency = efficiencyData.find(
                        (e) => e.name === item.name
                      );
                      const achievementPercentage =
                        efficiency?.achievement || 0;

                      let achievementClass = "text-red-500";
                      if (achievementPercentage >= 75)
                        achievementClass = "text-green-500";
                      else if (achievementPercentage >= 50)
                        achievementClass = "text-yellow-500";
                      ...
                      return (
                        ...
                        >
                          <td
                            className={`px-4 py-4 whitespace-nowrap text-sm font-medium ${tableRowText}`}
                          >
                            {item.name}
                          </td>
                          <td
                            className={`px-4 py-4 whitespace-nowrap text-sm ${tableRowText}`}
                          >
                          ...
                          <td
                            className={`px-4 py-4 whitespace-nowrap text-sm font-medium ${achievementClass}`}
                          >
                          ...
                          >
                            ₹
                            {Math.round(
                              efficiency?.perPersonCollection || 0
                            ).toLocaleString("en-IN")}
                          </td>
                          <td
                            className={`px-4 py-4 whitespace-nowrap text-sm ${tableRowText}`}
