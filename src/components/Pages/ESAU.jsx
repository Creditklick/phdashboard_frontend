...
//     if(50 < percent && percent<70) return 's';
//     if(70<percent && percent<89) return 'average Performer';
//     if (90<=percent && percent<100) return 'Excellent Performer';
...
//   const selectBgColor = darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-800';
//   const rowSelectedColor = darkMode ? 'bg-gray-700' : 'bg-blue-50';

//   const handleData = async () => {

//      if(!filters.process || !filters.startDate || !filters.endDate){
//           alert("Please Select Process and Date");
//           return;
//      }


//      try{
//            const response = await axios.get('https://phdashboard-backend.onrender.com/ph/api/esau/target',{filters});

//            if(response.data?.sccesss==true){
//                    console.log("Date was get in my frontend");
//            }
//      }
//      catch(err){
...
//             <h2 className="text-2xl font-bold mb-4">ESAU Dashboard</h2>

//        <button onClick={handleData} className='px-2 py-1 bg-amber-200 rounded-lg cursor-pointer shadow-2xs hover:bg-amber-400  border-2  ease-in-out duration-150'>Submit</button>
...
//             value={filters.am}
//             onChange={(e) => {
//               setFilters({ ...filters, am: e.target.value });
//             }}
//             className={`border ${borderColor} rounded-lg p-2 w-full ${selectBgColor}`}
//           >
...
//             value={filters.tl}
//             onChange={(e) => {
//               setFilters({ ...filters, tl: e.target.value });
//             }}
//             className={`border ${borderColor} rounded-lg p-2 w-full ${selectBgColor}`}
//           >
...
//             value={filters.esau}
//             onChange={(e) => {
//               setFilters({ ...filters, esau: e.target.value });
//             }}
//             className={`border ${borderColor} rounded-lg p-2 w-full ${selectBgColor}`}
//           >
...
function ESAU({ darkMode = false }) {
  ...
  const getESAUStatusString = (status) => {
    switch(status) {
      case 'U': return 'Under Performer';
      case 'S': return 'Satisfactory';
      case 'A': return 'Average Performer';
      case 'E': return 'Excellent Performer';
      case 'O': return 'Over Achiever';
    ...
    <div className={`p-6 min-h-screen ${bgColor} ${textColor}`}>
      ...
      {filteredData.length > 0 ? (
          ...
          <table className="min-w-full">
            ...
            <tbody>
              {filteredData.map((agent, index) => (
                <tr 
                  key={index} 
                  className={`${index % 2 === 0 ? rowBgColor : headerBgColor} border-b ${borderColor}`}
                >
                  ...
                  <td className="p-3 text-right">₹{agent.target.toLocaleString()}</td>
                  <td className="p-3 text-right">₹{agent.total_collected.toLocaleString()}</td>
                  <td className="p-3 text-right">₹{Math.abs(agent.rest).toLocaleString()}</td>
                  ...
                  <td className="p-3">
                    <div className="group relative inline-block">
                      ...
                      <div className={`absolute hidden group-hover:block z-10 p-2 rounded shadow-lg min-w-max ${
                        darkMode ? 'bg-gray-800' : 'bg-white'
                      } border ${borderColor}`}>
                        <div className="font-semibold mb-1">Collection Dates:</div>
        ...
        <div className={`text-center py-8 rounded-lg border ${borderColor}`}>
          {loading ? 'Loading data...' : 'No data available. Submit your query to see results.'}
        </div>
      )}
    </div>
  );
