  ...
  //   return (
  //     <div
  //       className={`fixed top-4 left-1/2 -translate-x-1/2 p-4 rounded-lg shadow-lg text-white ${bgColor} z-[100] text-center cursor-pointer transition-all duration-300 ease-in-out`}
  ...
  //   "http://172.16.0.175:5000/milestone/upload/milestones",
  //   formData,
  //   {
  //     headers: {
  ...
  //   const [activefile, setAddFile] = useState(false);
  ...
  //   // This useEffect fetches data from the API whenever the selected month or year changes
  //   useEffect(() => {
  ...
  //       try {
  //         const response = await axios.post(
  //           'http://172.16.0.175:5000/milestone/getdata/milestones',
  //           { month: selectedMonth, year: selectedYear }
  //         );
          
  //         // Assuming the API returns a response like { success: true, data: [...] }
  //         if (response.data.success && Array.isArray(response.data.data)) {
  //           // The API returns keys in snake_case, so we need to map them to camelCase
  ...
  //             allocationValueinCr: item.allocation_value_cr,
  //             milestones_8th: item.milestones_8th,
  //             milestones_14th: item.milestones_14th,
  //             milestones_21st: item.milestones_21st,
  //             milestones_28th: item.milestones_28th,
  //             achievement_8th: item.achievement_8th,
  //             achievement_14th: item.achievement_14th,
  //             achievement_21st: item.achievement_21st,
  //             achievement_28th: item.achievement_28th,
  //             achievement_percent_8th: item.achievement_percent_8th,
  //             achievement_percent_14th: item.achievement_percent_14th,
  //             achievement_percent_21st: item.achievement_percent_21st,
  //             achievement_percent_28th: item.achievement_percent_28th,
  //             sdlm_8th: item.sdlm_8th,
  //             sdlm_14th: item.sdlm_14th,
  //             sdlm_21st: item.sdlm_21st,
  //             sdlm_28th: item.sdlm_28th,
  //             sdbm_8th: item.sdbm_8th,
  //             sdbm_14th: item.sdbm_14th,
  //             sdbm_21st: item.sdbm_21st,
  //             sdbm_28th: item.sdbm_28th,
  //           }));
  //           setAllData(formattedData);
  //         } else {
  //           setAllData([]); // If no data, clear the table
  //           setFetchError('Failed to fetch data or data format is incorrect.');
  //         }
  //       } catch (err) {
  //         console.error("Error fetching milestone data:", err);
  //         setFetchError('Failed to fetch data. Please check your API.');
  ...
  //     fetchMileStoneData();
  //   }, [selectedMonth, selectedYear]); // Rerun the effect when month or year changes

    
  //   // This useEffect will run whenever allData changes to populate dropdowns and table
  //   useEffect(() => {
  //     // Reset filters when allData changes
  //     setFilters({
  //       clientName: '',
  //       product: '',
  //       bucket: '',
  ...
  //     setAddFile(!activefile);
  //   };

  //   // Callback function for a successful upload
  //   const handleUploadSuccess = (uploadedData) => {
  //     // The uploaded data might need to be filtered by the current month/year
  //     // Or we can simply refetch all the data to be safe.
  //     // For now, let's assume we get a single month's data on upload and add it to allData
  //     setAllData(prevData => [...prevData, ...uploadedData]);
  //     setAddFile(false); // Close the modal
  //   };

  //   const filterKeys = ['clientName', 'product', 'bucket', 'ph', 'aph'];

  //   const renderDropdown = (key) => {
  //     const items = dropdownData[`${key}s`];
  //     const keyName = key.charAt(0).toUpperCase() + key.slice(1).replace(/([A-Z])/g, ' $1');

  //     return (
  //       <div key={key} className="w-full">
  //         <select
  //           className="w-full h-10 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-200"
  //           value={filters[key]}
  ...
  //   // Function to render achievement percentage with color coding
  //   const renderAchP = (achieved, milestone) => {
  //     const numAchieved = Number(achieved);
  //     const numMilestone = Number(milestone);
      
  //     if (numMilestone === 0 || isNaN(numAchieved) || isNaN(numMilestone)) {
  //       return <span className="text-gray-500">N/A</span>;
  //     }
      
  //     const percentage = (numAchieved / numMilestone) * 100;
  //     let colorClass = '';
  //     if (percentage >= 50) {
  ...
  //   // Create a filtered table data based on the selected filters
  //   const filteredTableData = tableData.filter(row => {
  //     return (
  //       (!filters.clientName || row.clientName === filters.clientName) &&
  //       (!filters.product || row.product === filters.product) &&
  //       (!filters.bucket || row.bucket === filters.bucket) &&
  //       (!filters.ph || row.ph === filters.ph) &&
  //       (!filters.aph || row.aph === filters.aph)
  //     );
  //   });

  //   // Helper function to get the correct ordinal key suffix ('st' or 'th')
  ...
  //   return (
  //     <div className="p-4 sm:p-6 space-y-4 max-w-6xl sm:space-y-6 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 min-h-screen font-sans antialiased overflow-y-auto">
  ...
  //         <h2 className="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-white mb-2 sm:mb-0">Allocation Dashboard</h2>
  //         <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
  //           <div className="flex gap-2">
  //             <select
  //               value={selectedMonth}
  //               onChange={(e) => setSelectedMonth(e.target.value)}
  //               className="px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 font-semibold rounded-md shadow-md border border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500"
  //             >
  //               {months.map(month => (
  ...
  //         {filterKeys.map((key) => renderDropdown(key))}
  //         <button
  //           onClick={() => setFilters({
  //             clientName: '',
  //             product: '',
  ...
  //             aph: '',
  //           })}
  //           className="w-full col-span-2 md:col-span-1 h-10 px-4 py-2 bg-gray-200 text-gray-800 dark:bg-gray-700 dark:text-white font-semibold rounded-md shadow-md hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors duration-200"
  //         >
  //           Clear Filters
  //         </button>
  //       </div>
  ...
  //       {/* Data Table */}
  //       <div className="overflow-x-auto w-full max-w-full mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
  //         <table className="w-full text-left table-auto border-collapse">
  //           <thead>
  //             <tr>
  //               <th rowSpan="2" className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-200 dark:bg-gray-700 min-w-[120px]">Client Name</th>
  //               <th rowSpan="2" className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-200 dark:bg-gray-700 min-w-[100px]">Product</th>
  //               <th rowSpan="2" className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-200 dark:bg-gray-700 min-w-[100px]">Bucket</th>
  //               <th rowSpan="2" className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-200 dark:bg-gray-700 min-w-[80px]">PH</th>
  //               <th rowSpan="2" className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-200 dark:bg-gray-700 min-w-[80px]">APH</th>
  //               <th colSpan="2" className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-200 dark:bg-gray-700">Allocation</th>
  //               <th colSpan="4" className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-200 dark:bg-gray-700">{selectedMonth} Milestones</th>
  //               <th colSpan="4" className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-200 dark:bg-gray-700">Achievement</th>
  //               <th colSpan="4" className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-200 dark:bg-gray-700">Achievement %</th>
  //               <th colSpan="4" className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-200 dark:bg-gray-700">SDLM</th>
  //               <th colSpan="4" className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-200 dark:bg-gray-700">SDBM</th>
  //             </tr>
  //             <tr>
  //               <th className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-100 dark:bg-gray-600 min-w-[70px]">Count</th>
  //               <th className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-100 dark:bg-gray-600 min-w-[100px]">Value in Cr</th>
  //               {[8, 14, 21, 28].map((d) => (
  //                 <th key={`milestones_${d}`} className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-100 dark:bg-gray-600 min-w-[50px]">{d}{getOrdinalSuffix(d)}</th>
  //               ))}
  //               {[8, 14, 21, 28].map((d) => (
  //                 <th key={`achievement_${d}`} className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-100 dark:bg-gray-600 min-w-[50px]">{d}{getOrdinalSuffix(d)}</th>
  //               ))}
  //               {[8, 14, 21, 28].map((d) => (
  //                 <th key={`achievement_p_${d}`} className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-100 dark:bg-gray-600 min-w-[70px]">{d}{getOrdinalSuffix(d)}</th>
  //               ))}
  //               {[8, 14, 21, 28].map((d) => (
  //                 <th key={`sdlm_th_${d}`} className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-100 dark:bg-gray-600 min-w-[50px]">{d}{getOrdinalSuffix(d)}</th>
  //               ))}
  //               {[8, 14, 21, 28].map((d) => (
  //                 <th key={`sdbm_th_${d}`} className="p-2 border border-gray-300 dark:border-gray-600 text-center bg-gray-100 dark:bg-gray-600 min-w-[50px]">{d}{getOrdinalSuffix(d)}</th>
  //               ))}
  //             </tr>
  //           </thead>
  //           <tbody>
  ...
  //             ) : fetchError ? (
  //               <tr>
  //                 <td colSpan="19" className="px-4 py-8 text-center text-red-500">
  //                   Error: {fetchError}
  //                 </td>
  //               </tr>
  //             ) : (
  ...
  //                   <tr key={i} className="hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-150">
  //                     <td className="p-2 border border-gray-200 dark:border-gray-700">{row.clientName}</td>
  //                     <td className="p-2 border border-gray-200 dark:border-gray-700">{row.product}</td>
  //                     <td className="p-2 border border-gray-200 dark:border-gray-700">{row.bucket}</td>
  //                     <td className="p-2 border border-gray-200 dark:border-gray-700">{row.ph}</td>
  //                     <td className="p-2 border border-gray-200 dark:border-gray-700">{row.aph}</td>
  //                     <td className="p-2 border border-gray-200 dark:border-gray-700 text-center">{row.allocationCount}</td>
  //                     <td className="p-2 border border-gray-200 dark:border-gray-700 text-center">{row.allocationValueinCr}</td>
  //                     {/* FIX: Corrected the property access for '21st' milestone data. 
  //                       The original code used `milestones_${d}th` which is incorrect for `d=21`.
  //                       This change ensures the key matches the data structure (`milestones_21st`). */}
  //                     {[8, 14, 21, 28].map((d) => (
  //                       <td key={`milestone_${d}`} className="p-2 border border-gray-200 dark:border-gray-700 text-center">{row[`milestones_${d}${getOrdinalSuffix(d)}`]}</td>
  //                     ))}
  //                     {[8, 14, 21, 28].map((d) => (
  //                       <td key={`ach_${d}`} className="p-2 border border-gray-200 dark:border-gray-700 text-center">{row[`achievement_${d}${getOrdinalSuffix(d)}`]}</td>
  //                     ))}
  //                     {[8, 14, 21, 28].map((d) => (
  //                       <td key={`ach_p_${d}`} className="p-2 border border-gray-200 dark:border-gray-700 text-center">{renderAchP(row[`achievement_${d}${getOrdinalSuffix(d)}`], row[`milestones_${d}${getOrdinalSuffix(d)}`])}</td>
  //                     ))}
  //                     {[8, 14, 21, 28].map((d) => (
  //                       <td key={`sdlm_${d}`} className="p-2 border border-gray-200 dark:border-gray-700 text-center">{row[`sdlm_${d}${getOrdinalSuffix(d)}`]}</td>
  //                     ))}
  //                     {[8, 14, 21, 28].map((d) => (
  //                       <td key={`sdbm_${d}`} className="p-2 border border-gray-200 dark:border-gray-700 text-center">{row[`sdbm_${d}${getOrdinalSuffix(d)}`]}</td>
  //                     ))}
  //                   </tr>
  //                 ))
  //               ) : (
  ...
  //       {activefile && <UploadModal onClose={handleOpenState} onUploadSuccess={handleUploadSuccess} />}
  //     </div>
  //   );
  // }
...
// import { ChevronDown, X, Upload, Loader2 } from "lucide-react"; // Added X, Upload, Loader2 imports
// import axios from "axios";
...
//       aria-live="assertive" // Added for accessibility
//     >
//       {message}
//     </div>
//   );
...
//         aria-haspopup="listbox" // Added for accessibility
...
//             aria-label={`Search ${label}`} // Added for accessibility
//           />
//           {filteredOptions.length > 0 ? (
//             <ul role="listbox" className="py-1"> {/* Added for accessibility */}
//               {filteredOptions.map((option) => (
//                 <li
//                   key={option}
//                   className="flex items-center px-2 py-1 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer"
//                   role="option" // Added for accessibility
//                   aria-selected={selected.includes(option)} // Added for accessibility
//                   onClick={() => handleToggleOption(option)} // Added for better click target
//                 >
//                   <input
//                     type="checkbox"
...
//                     className="mr-2"
//                     id={`checkbox-${label}-${option}`} // Unique ID for label association
//                     tabIndex={-1} // Make checkbox not directly focusable as list item is clickable
//                   />
...
//   const getOrdinalSuffix = (day) => (day === 21 ? "st" : "th"); // Moved to be a pure function if not dependent on App state
...
//     // After successful upload, close the modal and re-fetch data
//     setShowUploadModal(false);
//     fetchMileStoneData(); // Re-fetch data to update the table
//   };
...
//                     >
//                       {row[`milestones_${d}${getOrdinalSuffix(d)}`]}
//                     </td>
//                   ))}
//                   {[8, 14, 21, 28].map((d) => (
//                     <td
...
//                       className="border px-2 py-1"
//                     >
//                       {row[`achievement_${d}${getOrdinalSuffix(d)}`]}
//                     </td>
//                   ))}
//                   {[8, 14, 21, 28].map((d) => (
//                     <td
...
//                       className="border px-2 py-1"
//                     >
//                       {row[`achievement_percent_${d}${getOrdinalSuffix(d)}`]}%
//                     </td>
//                   ))}
//                   {[8, 14, 21, 28].map((d) => (
//                     <td
...
//                       className="border px-2 py-1"
//                     >
//                       {row[`sdlm_${d}${getOrdinalSuffix(d)}`]}
//                     </td>
//                   ))}
//                   {[8, 14, 21, 28].map((d) => (
//                     <td
...
//                       className="border px-2 py-1"
//                     >
//                       {row[`sdbm_${d}${getOrdinalSuffix(d)}`]}
//                     </td>
//                   ))}
//                 </tr>
//               ))
...
//       return;
//     } else if (!file) {
//       setMessage("Please select an Excel file."); // More specific message
//       setMessageType("error");
//       return;
//     }
...
//     const formData = new FormData();
//     formData.append('file', file);
//     formData.append('date', selectedDate); // Ensure date is correctly formatted if needed by backend
...
//       // Assuming your API returns { success: true, message: "...", data: [...] }
//       setMessage(response.data.message || "File uploaded successfully!");
//       setMessageType("success");

//       // Call the success callback to trigger data re-fetch in parent
//       onUploadSuccess(); // No need to pass data back, parent will re-fetch all.

//       // Optionally clear form fields after successful upload
//       setFile(null); // Clear the file state
//       setSelectedDate(''); // Clear the selected date
//       // If you want the label to reset, you'd need to clear the input value too,
//       // but `setFile(null)` should suffice for state management.
...
//     } finally {
//       setLoading(false);
//       // The MessageToast handles its own dismissal.
//       // Do NOT close the modal here; let the user see the success/error message.
//     }
//   };

//   const handleClickDateInput = () => {
//     // Programmatically open date picker
//     dateInputRef.current?.showPicker();
//   };
...
//             // Make input visible but styled to appear as a custom button if showPicker isn't supported
//             className="absolute opacity-0 w-0 h-0 pointer-events-none"
//             aria-label="Date for upload" // Added for accessibility
//           />
...
//         <button
//           onClick={handleUpload}
//           disabled={loading || !file || !selectedDate} // Disable if no file or date
//           className={`w-full px-4 py-3 font-semibold rounded-md shadow-md transition-colors flex items-center justify-center gap-2 ${
//             loading || !file || !selectedDate ? 'bg-gray-300 text-gray-500 dark:bg-gray-500 dark:text-gray-300 cursor-not-allowed' : 'bg-green-600 text-white hover:bg-green-700'
//           }`}
//         >
