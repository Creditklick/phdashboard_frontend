    ...
    <div className={`p-6 ${bgColor} ${textColor} min-h-screen`}>
      ...
      {darkMode && (
        <div className="relative h-[300px] w-full mb-8 overflow-hidden rounded-xl">
          ...
          />
          <div className={`absolute inset-0 flex items-center justify-center ${cardBgColor} bg-opacity-70 rounded-xl`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {[
          { value: '1,248', label: 'Total Processes', change: '+12%', trend: 'up' },
          { value: '₹2.4M', label: 'Total Allocated', change: '+8.5%', trend: 'up' },
          { value: '₹1.8M', label: 'Total Recovered', change: '+5.2%', trend: 'up' },
          { value: '76.3%', label: 'Avg Recovery Rate', change: '-1.3%', trend: 'down' }
        ].map((stat, index) => (
          <div 
            key={index}
            className={`${statCardBg} ${statCardBorder} border rounded-xl p-6 shadow-sm transition-all hover:shadow-md`}
          >
            <div className={`text-3xl font-bold ${statValueColor} mb-2`}>{stat.value}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className={`${cardBgColor} ${borderColor} border rounded-xl p-6 lg:col-span-2`}>
            ...
            {['Under', 'Average', 'Excellent'].map((type) => (
              <div key={type} className="text-center">
                <div className={`text-lg font-medium ${
                  type === 'Under' ? (darkMode ? 'text-red-400' : 'text-red-600') :
                  type === 'Average' ? (darkMode ? 'text-yellow-400' : 'text-yellow-600') :
                  (darkMode ? 'text-green-400' : 'text-green-600')
                }`}>
                  {type === 'Under' ? '12%' : type === 'Average' ? '34%' : '54%'}
                </div>
            ...
            {[
              ...
              <div key={index} className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-sm text-gray-500">{performer.process}</div>
                </div>
      ...
      {/* Recent Activity */}
      <div className={`${cardBgColor} ${borderColor} border rounded-xl p-6 mt-6`}>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full">
            ...
            <tbody>
              {[
                { agent: 'Ravi', process: 'Proc A', amount: '₹12,500', status: 'Completed' },
                { agent: 'Neha', process: 'Proc B', amount: '₹8,200', status: 'In Progress' },
                { agent: 'Sita', process: 'Proc A', amount: '₹15,000', status: 'Completed' },
                { agent: 'Amit', process: 'Proc C', amount: '₹5,750', status: 'Pending' }
              ].map((item, index) => (
                <tr 
                  key={index} 
                  className={`${darkMode ? 'border-gray-700 hover:bg-gray-700/50' : 'border-gray-200 hover:bg-gray-50'} border-b`}
                >
