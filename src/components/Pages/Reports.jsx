...
import {
  FaUsers, FaUserCheck, FaBullseye, FaMoneyBillWave, FaFolderOpen,
  FaChartPie, FaDollarSign, FaUserTie, FaUserFriends, FaCalendarAlt,
  FaSync
} from 'react-icons/fa';
...
const Reports = ({ darkMode }) => {
  ...
  const fetchReports = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get('https://phdashboard-backend.onrender.com/ph/api/dashboarddata', {
        params: {
    ...
    <div className={`${darkMode ? 'dark' : ''}`}>
      <div className="p-6 min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
        <div className='flex justify-between items-center mb-6 flex-wrap gap-4'>
          <h2 className="text-2xl font-bold text-gray-800 dark:text-white">Process Reports</h2>
        ...
        {loading ? (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                  <div className="bg-indigo-50 dark:bg-gray-800 rounded-lg border dark:border-gray-600 p-5 flex items-center space-x-4">
                    ...
                    <div>
                      <p className="text-sm text-gray-500 dark:text-gray-400">Total AMs</p>
                      <p className="text-xl font-bold text-indigo-700 dark:text-indigo-300">{process.AMs}</p>
                    </div>
                  </div>
                  <div className="bg-indigo-50 dark:bg-gray-800 rounded-lg border dark:border-gray-600 p-5 flex items-center space-x-4">
                    <FaUserFriends className="text-indigo-500 text-2xl" />
                    <div>
                      <p className="text-sm text-gray-500 dark:text-gray-400">Total TLs</p>
                      <p className="text-xl font-bold text-indigo-700 dark:text-indigo-300">{process.TLs}</p>
                    </div>
                  </div>
                </div>
                  ...
                  {Object.entries(process.stats).map(([key, value]) => (
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        <div>{icons[key]}</div>
                        <div>
                          <p className="text-sm text-gray-500 dark:text-gray-400 capitalize">{formatLabel(key)}</p>
          ...
          <div className="text-center py-12">
            <p className="text-gray-500 dark:text-gray-400">No data available. Select dates and click Refresh to load data.</p>
          </div>
        )}
      </div>
    </div>
